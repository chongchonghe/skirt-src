...
sys.path.append(HOME + "/Sam/coding")
from pkgs import ramses
from pkgs.utils import utilities, units
# sys.path.append(HOME + "/Sam/work")
sys.path.append("/startrek2nb/chongchong/Sam/work")
#import radiation_module
import ef_qesc
from center import unit_lum
...
LOGGER.info("LOGGER info is on")
LOGGER.debug("LOGGER debug is on")
...
def make_spec(data_base_dir, plot_base_dir, ha_data, jobid, is_overwrite_json=0,
              axs=None, is_ret_ratio=False, feature=None, fn_fig="ha-{jobid}",
              boxw=1.0, data_name=""):

    r = ramses.Ramses(jobid, jobdir=f"{HOME}/Sam")
    data_job_dir = f"{data_base_dir}/Job{jobid}"
    #halpha_data_base_dir="../data/data_amr2cube/v7"
